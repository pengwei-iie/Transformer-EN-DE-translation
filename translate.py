...
def main(opt):
    translator = Translator(opt, use_cuda=False)
    print(torch.load(opt.vocab))
    ...
    with open(opt.decode_output, 'w', encoding='utf-8') as output:
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n_best', type=int, default=2, help='Output the n_best decoded sentence')
